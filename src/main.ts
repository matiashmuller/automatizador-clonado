...
async function main() {
  ...
  while (enMenu) {
    ...
    if (!datosCargados) {
      ...
      opcionesMenu.push(
        ...
        },
        {
          ...
          value:       'sincronizar',
          description: 'Escanea las carpetas y actualiza el JSON si pegaste entregas a mano.',
        },
        {
        ...
        },
        {
          ...
          description: 'Devuelve a PENDIENTE entregas archivadas y elimina su .md del historial.',
        },
        {
    ...
    // ── Mostrar menú con número de shortcuts ─────────────────────────────────
    let modoClonado: string | null;

    try {
      modoClonado = await menuSelect(
        datosCargados ? '¿Qué acción deseás realizar?' : '¡Bienvenido! Empecemos por cargar los datos:',
        opcionesMenu
      );
    } catch (e: any) {
      // ExitPromptError (Esc / Ctrl+C dentro de un sub-prompt): vuelve al menú
      if (e?.name === 'ExitPromptError') continue;
      throw e;
    }

    // null = Backspace en el menú principal → salir
    ...
    try {
      ...
      if (modoClonado === 'escanear') {
        ...
        if (config.ALL_COMISIONES) {
          ...
          console.log('⚡ Modo --all-comisiones: procesando todas las comisiones.\n');
        } else {
          comisionesElegidas = await checkbox({
            message: 'Seleccioná las comisiones a procesar:',
            choices: comisionesDisponibles.map(c => ({ name: c, value: c, checked: true })),
          });
        ...
        for (const p of presentesConfirmados) {
          ...
          if (!usuario) { sinMatch.push(p); continue; }
        ...
        if (matcheados.length > 0) {
          ...
          repoMap       = await fetchReposForUsers(matcheados.map(a => a.usuario['Usuario Github Registrado']));
          datosCargados = true;
      ...
      if (modoClonado === 'sincronizar') {
        ...
        const huboCambios =
          sincronizadosEnProceso > 0 || sincronizadosCorregidos > 0 ||
          sincronizadosArchivados > 0 || devueltosAPendiente > 0 || reabiertosDeArchivado > 0;

        if (huboCambios) {
          console.log('✅ Estados actualizados según carpetas locales:');
          if (sincronizadosEnProceso > 0)
            console.log(`   📂 ${sincronizadosEnProceso} pasaron a "En proceso".`);
          if (sincronizadosCorregidos > 0)
            console.log(`   ✅ ${sincronizadosCorregidos} detectadas como "Corregidas" (tienen nota).`);
          if (sincronizadosArchivados > 0)
            console.log(`   📦 ${sincronizadosArchivados} pasaron a "Archivado" (carpetas removidas con nota).`);
          ...
          if (reabiertosDeArchivado > 0)
            console.log(`   🔓 ${reabiertosDeArchivado} reabiertos desde "Archivado" → "En proceso" (y .md del historial eliminado).`);
        } else {
      ...
      if (modoClonado === 'gestionar_corregidos') {
        const activos = matcheados.filter(
          a => getAlumnoState(a.dni) === 'EN_CORRECCION' || getAlumnoState(a.dni) === 'CORREGIDO'
        );
        ...
        const alumnosSeleccionados = await checkbox({
          message: 'Marcá con Espacio los corregidos y desmarcá para revertir (Enter confirma):',
        ...
        // Desmarcar = revertir a EN_CORRECCION
        ...
        if (archivar) {
          ...
          if (archivadosExito.length > 0)
            console.log(`✅ Archivados con éxito: ${archivadosExito.length} entregas.`);
      ...
      if (modoClonado === 'reabrir_archivados') {
        const archivados = matcheados.filter(a => getAlumnoState(a.dni) === 'ARCHIVADO');
        ...
        for (const alumno of seleccionados) {
          // ① Eliminar el .md del historial (no debe quedar como "ya corregido")
          const eliminado = deleteHistorialMd(alumno);
          if (eliminado) eliminados++;

          // ② Volver a PENDIENTE para que pueda re-clonarse
          updateAlumnoState(alumno, 'PENDIENTE');
        }

        console.log(`\n🔓 ${seleccionados.length} entregas devueltas a "Pendiente".`);
        if (eliminados > 0)
          console.log(`   🗑️  ${eliminados} archivos eliminados de historial_correcciones.`);
        console.log('\n   Podés re-clonarlas con la opción "Automático" o "Buscar repo individual".');
        await input({ message: 'Presioná Enter para volver al menú...' });
        continue;
      }
        ...
        const alumnoElegido = await search({
          ...
          source:  async (inputStr) => {
            const opciones = matcheados.map(alumno => {
              ...
              const icono  = { EN_CORRECCION: '📂', CORREGIDO: '✅', ARCHIVADO: '🧹', PENDIENTE: '⬜' }[estado] ?? '⬜';
              return {
                name:  `${icono} ${alumno.nombre} (${alumno.dni}) [@${alumno.usuario['Usuario Github Registrado']}]`,
                value: alumno,
              };
            });
            if (!inputStr) return opciones;
            const term = inputStr.toLowerCase();
            return opciones.filter(op => op.name.toLowerCase().includes(term));
          },
        });
      ...
      if (modoClonado === 'auto' || modoClonado === 'paso_a_paso') {
        ...
        if (!cancelado) console.log('\n✅ Proceso por lotes finalizado.');
        await input({ message: 'Presioná Enter para volver...' });
        continue;
      }

    } catch (e: any) {
      // Si el usuario presiona Escape o Ctrl+C dentro de un sub-prompt de @inquirer,
      // vuelve silenciosamente al menú principal en lugar de crashear.
